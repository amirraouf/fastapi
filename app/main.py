...
from app.services.business_logic import list_transfer_logic, get_transfer, accept_transfer, deposit_balance, \
    reject_transfer, transfers_leaderboard
...
def reject_transfer_api(transfer_id: int, current_user: User = Depends(get_current_user), session: session_factory = Depends(get_session)):
    """
...
@app.post("/deposit")
def deposit_api(amount: Decimal, current_user: User = Depends(get_current_user), session: session_factory = Depends(get_session)):
...
@app.post("/withdraw")
def withdraw_api(amount: Decimal, current_user: User = Depends(get_current_user), session: session_factory = Depends(get_session)):
...
def get_top_transfers_api(limit: int = 10, by: str = "count", session: session_factory = Depends(get_session)):
    """
